...
export default function AgentsClient({ agents }: { agents: AgentWithDetails[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    switch (sort) {
      case 'popular':
        result = [...result].sort((a, b) => b.endorsement_count - a.endorsement_count);
        break;
      case 'rated':
        result = [...result].sort(
          (a, b) =>
            (b.agent_metrics[0]?.success_rate ?? 0) - (a.agent_metrics[0]?.success_rate ?? 0)
        );
        break;
      case 'newest':
        result = [...result].sort(
  ...
  return (
    <>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {filtered.map((agent) => (
          <AgentCard key={agent.slug} agent={toCardAgent(agent)} />
        ))}
      </div>
