...
function VerificationBadge({ level }: { level: Agent['verificationLevel'] }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-medium ${c.color} ${c.bg}`}
    >
      ...
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <span
              className={`rounded-md px-2 py-0.5 text-xs font-medium ${typeColors[blueprint.type]}`}
            >
            ...
            <div className="flex-1">
              ...
              <div className="mt-3 flex flex-wrap items-center gap-3">
                <span className="text-sm text-text-tertiary">
                  by <span className="text-text-primary font-medium">{agent.owner}</span> ·{' '}
                  {agent.ownerTitle}
                </span>
                <span className="text-text-tertiary">·</span>
                <span className="text-sm text-text-tertiary">Since {agent.operationalSince}</span>
                <span className="text-text-tertiary">·</span>
                <span className="text-sm text-text-tertiary">
                  {agent.endorsements} endorsements
                </span>
                <span className="text-text-tertiary">·</span>
                <span className="text-sm text-text-tertiary">{agent.reviewCount} reviews</span>
              </div>
          ...
          {activeTab === 'about' && (
            <div className="space-y-8">
              <div>
                ...
                <p className="leading-relaxed text-text-secondary">{agent.about}</p>
              </div>
            ...
            <div className="space-y-4">
              ...
              {agent.capabilities.map((cap) => (
                <div key={cap.name} className="flex items-center gap-4">
                  <span className="w-44 text-sm text-text-secondary shrink-0">{cap.name}</span>
                ...
                <div className="rounded-xl border border-border bg-surface-elevated p-6">
                  ...
                  <div className="mt-4 space-y-3">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-sm font-medium">{agent.metrics.avgResponseTime}</span>
                    </div>
                  </div>
                </div>
                <div className="rounded-xl border border-border bg-surface-elevated p-6">
                  ...
                  <div className="mt-4 space-y-3">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-sm font-medium">{agent.operationalSince}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-sm text-text-secondary">Endorsements</span>
                      <span className="text-sm font-medium">{agent.endorsements}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-sm text-text-secondary">Reviews</span>
                      <span className="text-sm font-medium">{agent.reviewCount}</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          {activeTab === 'blueprints' && (
            <div className="space-y-6">
              ...
              {agent.blueprints.length === 0 ? (
                ...
                agent.blueprints.map((bp, i) => <BlueprintCard key={i} blueprint={bp} />)
              )}
            </div>
          )}
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-0">
                {agent.activity.map((act, i) => (
                  <div key={i} className="flex gap-4 border-l-2 border-border py-4 pl-6 relative">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-xs text-text-tertiary">{act.date}</p>
                    </div>
                  </div>
                ))}
              </div>
